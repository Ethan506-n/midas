...
let locationProxy: any = null;
let originalLocation: Location | null = null;
let currentTargetUrl: string = '';

export function installLocationHook(proxyBase: string, targetUrl: string) {
  ...
  window.addEventListener('midas-location-update', (e: any) => {
  ...
  const locProxy = new Proxy(originalLocation!, {
    get(target, prop) {
      switch (prop) {
        ...
        case 'assign': return (url: string) => { navigate(url); };
        case 'replace': return (url: string) => { navigate(url, true); };
        case 'reload': return () => target.reload();
        default: return (target as any)[prop];
      }
    },
