...
let activeSandbox: HTMLIFrameElement | null = null;
let messageHandler: ((e: MessageEvent) => void) | null = null;

export function createSandbox(cfg: SandboxConfig): HTMLIFrameElement {
  destroySandbox();
...
function buildBootstrapHtml(cfg: SandboxConfig): string {
...
function handleSandboxMessage(data: any, cfg: SandboxConfig): void {
  if (!data || typeof data !== 'object') return;

  switch (data.type) {
    case 'sandbox-ready':
      if (cfg.onNavigate) cfg.onNavigate(data.url);
      break;
...
export function getSandboxFrame(): HTMLIFrameElement | null {
  return activeSandbox;
}
