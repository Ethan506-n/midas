...
interface NoiseConfig {
  ...
  minDecoyInterval: number;
  maxDecoyInterval: number;
...
export function initNoise(cfg: Partial<NoiseConfig> = {}): void {
  config = { ...config, ...cfg };
  if (config.enabled) startNoise();
}
...
function scheduleNextDecoy(): void {
  if (!isRunning) return;
...
function sendDecoyRequest(): void {
  try {
...
export function padPayload(data: ArrayBuffer): ArrayBuffer {
  if (!config.paddingEnabled) return data;
...
export function unpadPayload(data: ArrayBuffer): ArrayBuffer {
  if (!config.paddingEnabled) return data;
...
export function generateRandomTrafficBurst(count: number = 3): void {
