...
export function rewriteCaptchaScript(src: string, proxyBase: string): string {
  if (!isCaptchaUrl(src)) return src;
  return `${proxyBase}/_midas/passthrough?url=${encodeURIComponent(src)}`;
}

export function installCaptchaHooks(proxyBase: string) {
